...
/*
class MazeCreator : public Solver
{
public:
	MazeCreator(const StageData &stageData, const LevelData &levelData) : Solver(stageData, levelData), count(0) {}
	MazeCreator(const StageData &stageData, const LevelData &levelData, int money) : Solver(stageData, levelData, money), count(0) {}
	void run(int point);
...
private:
	...
	void upgradeCir(const Point &o, int r);
...
bool MazeCreator::canMove(const MapData &route, const Point &o, int i) const
{
...
void MazeCreator::createOnlyGoal()
{
	using namespace mark;
...
Task MazeCreator::createTask(const Point &p)
{
	count++;
	if(count > 0) count = 0;
...
vector<vector<int>> MazeCreator::createDistMap(const Point &goal, int kind)
{
...
void MazeCreator::debugPrint(const vector<vector<int>> &map) const
{
...
const Point MazeCreator::createMaze()
{
	using namespace mark;
	const MapData &map = getMap();
...
void MazeCreator::run(int point)
{
	...
	if(DEBUG_FLAG)
	{
		int damage = simulate().size();
		static int life = 9;
		cout << "Simulate Life : " << life << endl;
		life-=damage;
		if(damage) cout << "\x1b[31m" << "Damage : " << damage << "\x1b[39m" <<  endl;
		cout << endl;
	}
}

void MazeCreator::upgrade(int r, const Point &goal, const set<Point> &starts)
{
...
void MazeCreator::upgradeCir(const Point &o, int r)
{
	typedef pair<int, Point> NODE;
	priority_queue<NODE, vector<NODE>, greater<NODE>> que;
	MapData checkedmap(stage.map.width, vector<char>(stage.map.height, 0));
